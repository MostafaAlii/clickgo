...
<div id="kt_content_container" class="container-xxl">
    <div class="mb-5 card card-xxl-stretch mb-xl-8">
        ...
        <div class="card-body py-3">
            <form action="{{ route('admin.professions.update', $profession->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" id="{{ $key }}[name]" name="{{ $key }}[name]" class="form-control"
                                        value="{{ $profession->translateOrNew($key)->name }}" required>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="row mt-4">
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status" class="form-select">
                            <option value="active" {{ $profession->status == 'active' ? 'selected' : '' }}>نشط</option>
                            <option value="inactive" {{ $profession->status == 'inactive' ? 'selected' : '' }}>غير نشط
                            </option>
                        </select>
                    </div>
                </div>
                            ...
                            <div class="col-md-4 pt-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="documents[]" value="{{ $document->id }}"
                                        id="doc_{{ $document->id }}" {{ $profession->documents->contains($document->id) ? 'checked' : ''
                                    }}>
